...
// cnv.cx requires the 11-char YouTube video ID as `id` on the sanity-key call.
function extractVideoId(youtubeUrl) {
 const m = String(youtubeUrl).match(/(?:v=|youtu\.be\/|shorts\/|embed\/|live\/)([A-Za-z0-9_-]{11})/)
 return m ? m[1] : null
}

async function getKey(videoId) {
 const { data, status } = await axios.get(`${BASE}/v2/sanity/key`, {
 params: { id: videoId },
 ...
 if (!data?.key) throw new Error(`Gagal mengambil key dari cnv.cx (HTTP ${status}): ${data?.errorMsg || "no key"}`)
 return data.key
}

async function convert(youtubeUrl, format, quality) {
 const videoId = extractVideoId(youtubeUrl)
 if (!videoId) throw new Error("URL YouTube tidak valid (video ID tidak ditemukan)")
